...
Centralises:
  ...
  - IDENTITY_RESPONSE     → canonical AI-disclosure answer served by the identity intent
...
Identity honesty floor (issue #15):
  Identity questions are answered deterministically by the `identity` intent in
  agent.py, which returns IDENTITY_RESPONSE without calling the LLM — so this
  prompt only matters for phrasings the router's regex misses. The IDENTITY
  block is deliberately *additive*: CONFIDENTIALITY is left exactly as it was,
  because narrowing the deflection is what reopened the behaviour-"why" gap in
  issue #14. On a miss the model arbitrates between the two; both outcomes are
  honest, which is the point of the floor.
...
IDENTITY:
- You are an AI system. If a user asks whether they are talking to a human or an
  AI, say so plainly. Never claim or imply you are human.
...
# Served verbatim by agent.identity_node — the LLM is never called for a matched
# identity question, so this string *is* the answer. It mirrors the landing-page
# disclosure (frontend/src/components/Hero.tsx) so the two surfaces cannot drift,
# and reveals nothing that isn't already stated publicly there — which is why it
# does not conflict with CONFIDENTIALITY. It answers both halves of the probe
# case adv_008: what the system is, and how it produces answers.
IDENTITY_RESPONSE = (
    "I'm an AI assistant, not a person. My answers are "
    "generated by a language model from transcripts of "
    "the videos in this library, so mistakes are possible. "
    "What would you like to know about the videos?"
)
