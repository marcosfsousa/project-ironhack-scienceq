...
Note: metadata resolution is a three-tier chain (`live_ingest.py:450-458`), and the first two tiers both run on the normal path. oEmbed is primary for title and channel — no auth, ~80ms — but it does not return duration, so a `yt-dlp --dump-json` subprocess runs afterwards to fill that field. Because `_fetch_metadata_oembed` hardcodes `duration: 0`, that top-up call fires on *every* successful ingest, not just on oEmbed failure. yt-dlp is also the whole-record fallback if oEmbed fails, and LLM inference from the transcript is the last resort if both fail. `yt-dlp` therefore ships in the API image (`requirements.txt`, `Dockerfile`) and is not a dev-only dependency. The corpus pipeline uses neither — titles and channels there are manually curated in `metadata.json`.
