...
1. Fetch video metadata — title and channel via the YouTube oEmbed API, duration via `yt-dlp`
...
Note: metadata resolution is a three-tier chain (`live_ingest.py:451-460`), and the first two tiers both run on the normal path. oEmbed is primary for title and channel — no auth, ~80ms — but it does not return duration, so a `yt-dlp --dump-json` subprocess runs afterwards to fill that field. Because `_fetch_metadata_oembed` hardcodes `duration: 0`, that top-up call fires on *every ingest that reaches the metadata step*, not just on oEmbed failure. A duplicate URL is the exception: the cross-namespace check returns `success=True` before any metadata fetch, so a re-submitted video costs no network calls at all. yt-dlp is also the whole-record fallback if oEmbed fails, and LLM inference from the transcript is the last resort if both fail. `yt-dlp` therefore ships in the API image (`requirements.txt`, `Dockerfile`) and is not a dev-only dependency. The corpus pipeline uses neither — titles and channels there are manually curated in `metadata.json`.
...
**Cohere Rerank v3.5 (optional)** — a cross-encoder reranker sits between Pinecone retrieval and the LLM. Bi-encoder cosine similarity (used at Pinecone query time) is fast but imprecise — it encodes query and document independently. A cross-encoder like Cohere Rerank jointly attends to both, producing more accurate relevance scores at the cost of one additional API call per query. The design over-retrieves 10 candidates from Pinecone (`RETRIEVER_FETCH_K`), reranks them, and passes the top 3 to the LLM (`RETRIEVER_TOP_N`) — both values set by the Phase 5 sweep. Toggled via `RERANKER_ENABLED` env var to enable A/B comparison without code changes; on by default, and confirmed on in production (verified against the serving Cloud Run revision, 30 July 2026). The sweep-calibrated `RETRIEVER_FETCH_K`/`RETRIEVER_TOP_N` pair assumes it: with the reranker off, `retrieve()` ignores `FETCH_K` and fetches `top_k` directly. Impact is quantified by `eval/sweep_reranker.py`, which runs the full eval set with and without the reranker and outputs a per-dimension side-by-side table.
...
This table stops at gpt-oss-120b; [`EVALUATION.md`](EVALUATION.md) carries the current one, two checkpoints longer, plus the provenance of every row. None of the rows here has a recorded retrieval config, and the bolded `gpt-oss-120b` row has no committed results file either — treat all of them as directional rather than reproducible.

Phase 3 and Phase 4 scores are not comparable to prompt-v1/v2 — different embedding space (Cohere 1024d vs MiniLM 384d) and different eval methodology. Phase 6 added 8 non-English videos (ES/DE/FR/PT); cross-lingual retrieval validated via `eval/validate_multilingual.py` — 4/4 validation queries PASS with all non-English target chunks scoring above the 0.40 gate in force for that run (range: 0.52–0.70). Phase 5's sweep had already selected 0.25 by then, but it landed only in `.env.example`; the code fallback still read 0.40, which is the value `validate_multilingual.py` inherited. Both numbers are below the observed 0.52 floor, so the PASS result is unaffected. The fallbacks were reconciled to the sweep winners afterwards — see the score threshold gate above for the current 0.25. Frontend confirmed: English queries surface non-English source pills alongside English results (verified at the time in the since-retired Streamlit UI). prompt-v3 replaced the prohibition-framed grounding rule with a verification frame ("which excerpt supports this?") and an explicit inference ban — grounding +0.32 on multilingual cases, correctness +0.12 overall, no regressions on tone or conciseness. Two cases (ml_007 microplastics, ml_008 neurodivergence) remained at grounding=2 — a corpus data gap, not fixable by prompt alone. gpt-oss-120b swapped the LLM from `llama-3.3-70b-versatile` to `openai/gpt-oss-120b` (via Groq) with no prompt or retrieval changes — the largest single-checkpoint gain in the table: grounding +0.64, conciseness +0.91, correctness +0.16, overall mean +0.40 vs. prompt-v3. Tone dipped -0.12. The two previously stuck multilingual cases (ml_007, ml_008) recovered: grounding 2 → 5 and 2 → 4 respectively.
